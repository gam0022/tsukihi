#ifndef _PATHTRACING_RADIANCE_H_
#define _PATHTRACING_RADIANCE_H_
...
// ray方向からの放射輝度を求める
Color radiance_by_pathtracing(const Ray &ray, Random *rnd, const int depth) {
	...
	case REFLECTION_TYPE_DIFFUSE: {
		...
		incoming_radiance = radiance_by_pathtracing(Ray(hitpoint.position, dir), rnd, depth+1);
	...
	case REFLECTION_TYPE_SPECULAR: {
		...
		incoming_radiance = radiance_by_pathtracing(Ray(hitpoint.position, ray.dir - hitpoint.normal * 2.0 * dot(hitpoint.normal, ray.dir)), rnd, depth+1);
		weight = now_object->color / russian_roulette_probability;
	} break;
	...
	case REFLECTION_TYPE_REFRACTION: {
		...
		if (cos2t < 0.0) { // 全反射
			incoming_radiance = radiance_by_pathtracing(reflection_ray, rnd, depth+1);
			weight = now_object->color / russian_roulette_probability;
			break;
		}
		// 屈折の方向
		...
		if (depth > 2) {
			if (rnd->next01() < probability) { // 反射
				incoming_radiance = radiance_by_pathtracing(reflection_ray, rnd, depth+1) * Re;
				weight = now_object->color / (probability * russian_roulette_probability);
			} else { // 屈折
				incoming_radiance = radiance_by_pathtracing(refraction_ray, rnd, depth+1) * Tr;
				weight = now_object->color / ((1.0 - probability) * russian_roulette_probability);
			}
		} else { // 屈折と反射の両方を追跡
			incoming_radiance = 
				radiance_by_pathtracing(reflection_ray, rnd, depth+1) * Re +
				radiance_by_pathtracing(refraction_ray, rnd, depth+1) * Tr;
			weight = now_object->color / (russian_roulette_probability);
		}
	} break;
